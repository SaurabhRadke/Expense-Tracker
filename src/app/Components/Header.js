    ...
    return(
        <header className=" relative w-screen h-[14%] flex items-center border-b-zinc-400 px-10 justify-between">
            ...
            <div className="flex gap-[1.5vw] items-center">
                <div id="Getstart" className=" sm:block hidden"><Link href={authenticated?"/Dashboard":"/"}><button  className="  tracking-wider bg-black border-[1px] border-black px-4 py-2 text-white rounded-md hover:bg-white hover:text-black duration-700">Get Started</button></Link></div>
                {authenticated?<div id="signOut" onClick={()=>HandelLogout()}><Button  text="Sign Out"/></div>:<div id="sign" onClick={()=>HandelLogin()}><Button  text="Sign In"/></div>}
            </div>
            <div id="head-border" className=" absolute w-full h-0 border-[0.2px] border-zinc-600 bottom-0 left-0 "></div>
        </header>
    )
}